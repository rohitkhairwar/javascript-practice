...
In other words, all values present from all arrays should be included in their original order, but with 
...
function uniteUnique(...arr) {
    let mainArr = [];
    let paramArr = arr.flat();
    for (let i = 0; i < paramArr.length; i++) {
        if (mainArr.indexOf(paramArr[i]) === -1) {
            mainArr.push(paramArr[i]);
        }
    }
    return mainArr;
  }
